 ...
 * @type {{colorSchemes: {warn: {background: string, foreground: string}, finer: {background: string, foreground: string}, debug: {background:
...
export class Log {
  ...
  protected errorImpl(data: string | any, logMethodAndScheme: keyof LogLevel) {
    if (typeof data === 'string') {
      ...
      Log.console[logMethodAndScheme](inspect(data, false, 10, true));
    }
  }
  ...
  protected _log(data: any, logMethod: keyof LogLevel, scheme: keyof LogLevel | string) {
    if (typeof data === 'string') {
      ...
      Log.console[logMethod](inspect(data, false, 10, true));
    }
  }
}
